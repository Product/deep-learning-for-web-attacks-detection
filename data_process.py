...
# the old one 
def Find_keywords(file_in,file_save_keyword):
    '''
    find small seet of key words from statistics
    Counts the frequency of all words below and extract the top num of words 
    ----
     file_in: file we will extract key words from.  
     num: the num of words will be extracted (changable, defaults=100)
    ...
    # dict_out='Extract-Source/keywords.csv'
    ...
    for i in range(all_num):
        if sorted_freq[i]==0:
            num=i
            break
    print('key words number:', num)
    # num=sorted_freq.index(0)
    # print(num)      
    ...
    small_dicts = [word[0] for word in sorted_dicts[0:num]] # delete the space
    samllkey_words=" ".join(small_dicts)
    file_save=open(file_save_keyword,'wt')
    file_save.writelines(samllkey_words)
    # print (small_dicts)
    return small_dicts
...
def Data_mix(file1_in,file2_in,total_number,normal_number,file_out,label_out):
    ...
    ------
     Arg:
      file1_in:  the normal data (label: 0,1)
      file2_in:  the anomalous data (label: 1,0)
...
if __name__ == "__main__":
    ...
    Root_folder='ALL-New-V2/'


    # step 2  ------------------------  mix anomalous and normal data -----------------------
    ...
    # step 5 --------------- divide mixed data into train and test data -----------------------
    ...
    Extract_Train_Test(file_in,label_in,file_train,label_train,file_test,label_test,number=79146)
    # ---------------------------- end ------------------------------

    # step 6 ------------------ find key words -------------------
    ...
    # step 6 -------------- train the word2vec model (Only use train data)---------------------
    ...
    # step 6 -------------- train the FastText model (Only use train data)---------------------
        # file_in=Root_folder+'Replace-data/Replace_train.csv'
        # models_out=Root_folder+'Model-Fast/fasttext'
        # FastText_Save(file_in,models_out,0,48,100)
        # ------------------------------------------
        # model=FastText.load(models_out)
        # print (model.most_similar('<'))

        # step 7 -------------- count the length of every URL ------------------
        # file_name='Extract-Source-New/Train&Test/data-mixed.csv'
        # count_max_min(file_name)
        # count_stage(file_name,80)  #set vector as 80*48
